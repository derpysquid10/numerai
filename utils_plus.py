...
########################################################################################################################
# Acknowledgement: All functions before this are provided in the official Numerai data set

napi = numerapi.NumerAPI()

def download_data():
    """Downloads the latest Numerai data under ./data
    """
    print('Downloading dataset files...')
    # for file in "training", "validation", "test", "features.json":
    #     napi.download_latest_data(file, "parquet", "./data")

    Path("./v4").mkdir(parents=False, exist_ok=True)
    napi.download_dataset("v4/train_int8.parquet", dest_path="./data/train_int8.parquet")
    napi.download_dataset("v4/validation_int8.parquet", dest_path="./data/validation_int8.parquet")
    napi.download_dataset("v4/features.json", dest_path="./data/features.json")
    napi.download_dataset("v4/live_int8.parquet", dest_path="./data/live_int8.parquet")
    

    print("Done!")


def read_learning_data(features, training_data_path="./data/train_int8.parquet"
                       , validation_data_path="./data/validation_int8.parquet"):
...
def read_prediction_data(features, training_data_path="./data/train_int8.parquet"
                       , validation_data_path="./data/validation_int8.parquet"
                       , live_data_path = "./data/live_int8.parquet"):
